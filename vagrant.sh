...
# create nginx config
if [ ! -f /etc/nginx/sites-enabled/yii2.dev ]; then
    cp /var/www/vhost.conf.dist /var/www/vhost.conf
    sudo ln -s /var/www/vhost.conf /etc/nginx/sites-enabled/yii2.dev
fi

# Configuring application
echo "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY 'root'" | mysql -uroot -proot
echo "FLUSH PRIVILEGES" | mysql -uroot -proot
echo "CREATE DATABASE IF NOT EXISTS \`yii2\` CHARACTER SET utf8 COLLATE utf8_unicode_ci" | mysql -uroot -proot
